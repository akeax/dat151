...
acceptNoInterpreter :: Bool
acceptNoInterpreter = False
...
welcome :: IO ()
welcome = putStrLn $ "This is the test program for Programming Languages Lab 3"


runMake :: FilePath -> IO ()
runMake dir = do checkDirectoryExists dir
                 runCommandNoFail_ (dir ++ "c.bat") ""
...
testBackendProg prog f =
    do input  <- readFileIfExists (f++".input")
       ...
       if out == output || (acceptNoInterpreter && out == "no interpreter yet\n")
         then return True
